...
#[cfg(test)]
mod tests {
    use crate::builtins::bf_strings::{strsub, moo_value_to_json, json_value_to_moo};
    use moor_var::{Associative, v_int, v_str, v_list, v_map};
    ...
    fn test_moo_to_json_complex() {
        ...
        assert_eq!(moo_value_to_json(&list_val).unwrap(), json!([1, 2, "three"]));

        // Test map
        let map_val = v_map(&[(v_str("key1"), v_int(1)), (v_str("key2"), v_str("value"))]);
        assert_eq!(moo_value_to_json(&map_val).unwrap(), json!({"key1": 1, "key2": "value"}));
    }
    ...
    fn test_json_to_moo_primitives() {
        ...
        assert!(matches!(json_value_to_moo(&null_json).unwrap().variant(), moor_var::Variant::None));
