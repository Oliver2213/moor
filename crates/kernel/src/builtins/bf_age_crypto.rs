...
fn bf_age_encrypt(bf_args: &mut BfCallState<'_>) -> Result<BfRet, BfErr> {
    ...
    for recipient_var in recipients_list.iter() {
        ...
        // Try to parse as an SSH public key
        match PublicKey::from_openssh(recipient_str) {
            Ok(ssh_key) => {
                // Convert SSH key to string format that age can parse
    ...
    if let Err(e) = writer.finish() {
        error!("Failed to finalize encryption: {}", e);
        return Err(BfErr::Code(E_INVARG));
    }
            ...
            Err(_) => {
                ...
                match ssh_key::PrivateKey::from_openssh(key_str) {
                    Ok(ssh_key) => {
                        ...
                        let ssh_key_str = ssh_key.to_openssh(ssh_key::LineEnding::LF).unwrap_or_default();
                        // Use from_buffer to create the identity
                        match age::ssh::Identity::from_buffer(std::io::Cursor::new(ssh_key_str), None) {
                            Ok(ssh_identity) => {
